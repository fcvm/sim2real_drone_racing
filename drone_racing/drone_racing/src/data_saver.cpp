...
// public: Con- and destructors
    ...
    DataSaver::DataSaver(const ros::NodeHandle& nh, const ros::NodeHandle& pnh) 
        ...
        nh_(nh), 
        pnh_(pnh), 
        model_based_prediction_(Eigen::Vector3d::Zero()), 
        network_selection_(Eigen::Vector3d::Zero()), 
        run_idx_(5000), 
        frame_counter_(0), 
...
// public: Methods
    ...
    void DataSaver::saveImage
    (
        const DataSaver::trajectory_info traj_coor,
        const int model_based_frame
    ) 
    {
        if (record_data_) 
        {
            // create string representing run_idx_
            std::ostringstream ss_run;
            ss_run << std::setw(4) << std::setfill('0') << run_idx_;
            std::string s_run(ss_run.str());

            // create string representing frame_counter
