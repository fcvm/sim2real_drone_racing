...
// public: Methods
    ...
    void GlobalTrajectory::generateGlobalTrajectory (
    ...
    ) 
    {
        ...
        Eigen::Vector3d last = 999 * Eigen::Vector3d::Ones(); // [999, 999, 999]
        ...
        ROS_INFO (
            "Generating global trajectory through [%d] waypoints.", 
            static_cast<int>( waypoints_filtered.size() )
        );
        ...
        if ( !load_existing_trajectory_ ) 
        {
            ...
            trajectory = polynomial_trajectories::minimum_snap_trajectories::generateMinimumSnapRingTrajectoryWithSegmentRefinement (
                segment_times, 
                trajectory_settings, 
                global_traj_max_v_, 
                maximal_des_thrust,
            ...
            ROS_INFO (
                "Done with trajectory! flight time: %1.1f", 
                trajectory.T.toSec()
            );
            ...
            {
                t = i * dt;
                state = polynomial_trajectories::getPointFromTrajectory (
                    trajectory, 
                    ros::Duration( t )
                );
    ...
    quadrotor_common::TrajectoryPoint GlobalTrajectory::getNextStateOnGlobalTraj( const double horizon ) 
    {
        int forecast = 1;
        int eff_index = ( global_traj_idx_proj_ + forecast ) % global_trajectory_sampled_.size();

        while (
            ( global_trajectory_sampled_[eff_index].position - global_trajectory_sampled_[global_traj_idx_proj_].position).norm() < horizon
        ) 
        {
            forecast++;
    ...
    quadrotor_common::TrajectoryPoint GlobalTrajectory::projectOnTrajectory( quadrotor_common::TrajectoryPoint curr_desired_state ) 
    {
        tracking_dir_proj_ = 
              global_trajectory_sampled_[ global_traj_idx_proj_   ].position 
        ...
        if (
            ( curr_desired_state.position - global_trajectory_sampled_[global_traj_idx_proj_].position ).norm() > 1.0
        ) 
        {
    ...
    void GlobalTrajectory::findStartIndexGlobalTrajectory( const Eigen::Vector3d est_pos ) 
    {
        ...
        for (
            int i = 0; 
            i < global_trajectory_sampled_.size(); 
            i++
        ) 
        {
...
// private: Methods
    ...
    void GlobalTrajectory::saveGlobalTrajectory() 
    {
        ...
        ROS_INFO(
            "Saved global trajectory to %s", 
            filename_trajectory.c_str()
        );
    }
    ...
    void GlobalTrajectory::loadGlobalTrajectory() 
    {
        ...
        ROS_INFO (
            "Loading saved trajectory from %s", 
            filename_trajectory.c_str()
        );
