...
// public: Con- and destructor
        ...
        pnh_(nh_private) 
    {
        ...
        gazebo_model_states_sub_ = nh_.subscribe (
            "/gazebo/model_states", 
            1, 
            &GateReplacer::gazeboModelStatesSub,
            this
        );

        replace_gate_sub_ = nh_.subscribe (
            "/replace_gates", 
            1, 
            &GateReplacer::replaceGatesCallback, 
            this
        );

        replace_gate_pub_ = nh_.advertise<gazebo_msgs::ModelState> (
            "/gazebo/set_model_state", 
            0
        );

        ROS_INFO (
            "[%s] Gate replacer started!",
    ...
    void GateReplacer::makeGatesMove() 
    {
        ...
        for ( auto &i : model_states_msg_.name ) 
        {
            if ( i.find("gate") != std::string::npos ) 
            {
                ...
                gates_positions_temp_[goal_idx] 
                    = gates_positions_original_[goal_idx] 
                    + Eigen::Vector3d ( 
                        max_dynamic_amp_      * std::sin( speed_ * t + random_phases_[goal_idx].x() ),
                        max_dynamic_amp_      * std::sin( speed_ * t + random_phases_[goal_idx].y() ),
                        0.5*max_dynamic_amp_  * std::sin( speed_ * t + random_phases_[goal_idx].z() )
                    );
...
// private: Methods
        ...
        if ( !found_all_gates_ ) 
        {
            ...
            for ( auto &i : model_states_msg_.name ) 
            {
                ...
                ) 
                {
                    ...
                    {
                        ROS_INFO (
                            "[%s] All the gates have been found",
                            ros::this_node::getName().c_str()
                        );
        ...
        for ( auto &i : model_states_msg_.name ) 
        {
            if ( i.find("gate") != std::string::npos ) 
            {
                ...
                model_state.pose.position.x 
                    = only_gates_[goal_idx].x() 
                    + 2 * amplitude * ( ( (double) random() /RAND_MAX ) - 0.5 );
                model_state.pose.position.y 
                    = only_gates_[goal_idx].y() 
                    + 2 * amplitude * ( ( (double) random() /RAND_MAX ) - 0.5 );
                model_state.pose.position.z = std::max (
                    1.2, 
                    only_gates_[goal_idx].z() + 2 * amplitude * ( ( (double) random() /RAND_MAX ) - 0.5 )
                );
    ...
    void GateReplacer::loadGoalPositions() 
    {
        ...
        {
            ROS_ERROR (
                "[%s] Could not load goal positions list", 
        ...
        if (
            !pnh_.getParam (
                "goal_orientations", 
                raw_orientations_list
            )
        ) 
        {
            ROS_ERROR (
                "[%s] Could not load goal orientations list", 
                pnh_.getNamespace().c_str()
            );
        }
        ...
        ) 
        {
            ...
            gates_orientations_original_.push_back (
                static_cast<double>( raw_orientations_list[i]["yaw"] )
            );

            ROS_INFO (
                "Goal position: [%f, %f, %f][%f]", 
                static_cast<double>( raw_positions_list   [i]["x"]  ),
                static_cast<double>( raw_positions_list   [i]["y"]  ),
                static_cast<double>( raw_positions_list   [i]["z"]  ),
                static_cast<double>( raw_orientations_list[i]["yaw"])
            ...
            gates_velocities_.emplace_back (
                Eigen::Vector3d::Zero()
            );
            ...
            if (
                static_cast<double>( raw_positions_list[i]["gate"] ) > 0.5
            ) 
            {
