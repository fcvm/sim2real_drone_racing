...
// public: Con- and destructors
    ...
    DroneRacing::DroneRacing(
        ...
        state_estimate_quad_(), 
        state_estimate_world_(),
        state_machine_(State::kOff), 
        ...
        tf_listener_() 
    {
        ...
        start_navigation_sub_ = nh_.subscribe( "start_navigation",   1,  &DroneRacing::startNavigationCallback,  this );
...
// public: Methods
    ...
    Eigen::Vector3d DroneRacing::getEstimatedPos() 
    {
        ...
        Eigen::Vector3d est_pos = T_W_Q_.getPosition();
...
// private: Callback methods
    ...
    void DroneRacing::setupEnvironmentCallback( const std_msgs::EmptyConstPtr& msg )
    {
        ...
        global_trajectory_->generateGlobalTrajectory(
            visualizer_, 
            goal_positions_
        );
    ...
    {
        ROS_INFO (
            "Asked teacher [%d] times for help and executed [%d] times NN selection.", 
...
// private: Methods
    ...
    void DroneRacing::mainloop( const ros::TimerEvent& time ) 
    {
        ...
        if ( environment_is_set_up_ ) 
        {
            if (
                moving_gates_ 
                && 
                ( ros::WallTime::now() - start_time_ ).toSec() > sec_no_record_at_start_
            ) 
            {
    ...
    void DroneRacing::performNavigation() 
    {
        ...
        {
            ROS_WARN(
                "[%s] Desired state and state estimate diverged.", 
                ros::this_node::getName().c_str()
            );
            ROS_WARN(
                "Desired state: [%f, %f, %f]", 
    ...
    void DroneRacing::collectData() 
    {
        ...
        if ( mainloop_iter_ % plan_every_n_iter_ == 0 ) 
        {
            ...
            double v_execute = std::min (
                desired_state_world_.velocity.norm() + 0.5, 
                v_rescaled
            );
            ...
            double pitch_to_end = std::atan2 (
                T_B_goal.getPosition().z(),
                dist_to_goal
            );
            ...
            data_saver_->setModelBasedPrediction (
                Eigen::Vector3d (
                    traj_info.idx_right,
                    traj_info.idx_up,
                    traj_info.curr_vel
            ...
            if ( success ) 
            {
                trajectory_manager_->setBestTraj (
                    0, 
                    false
                );
            }
    ...
    bool DroneRacing::checkStateEstDesStateDivergence() 
    {
        ...
        divergence_vec = Eigen::Vector3d (
            desired_state_world_.position.x() - state_estimate_world_.pose.pose.position.x,
        ...
        std_msgs::Float64 divergenceVectorNorm;
        divergenceVectorNorm.data = divergence_vec.norm();
        divergence_pub_.publish( divergenceVectorNorm );
    ...
    void DroneRacing::transformToQuadFrame() 
    {
        ...
        const Eigen::Quaterniond q_W_O = T_W_S_.getEigenQuaternion();
        const Eigen::Vector3d t_W_O = T_W_S_.getPosition();
        const double scale_W_O = 1.0;
    ...
    void DroneRacing::transformStateEstimateToWorldFrame() 
    {
        ...
        state_estimate_world_.pose.pose.position = quadrotor_common::vectorToPoint (
            quadrotor_common::eigenToGeometry (
                1.0 / scale_W_O * ((q_W_O * quadrotor_common::geometryToEigen ( state_estimate_quad_.pose.pose.position )) + t_W_O)
            )
        );
        ...
        state_estimate_world_.pose.pose.orientation = quadrotor_common::eigenToGeometry(
            q_W_O * quadrotor_common::geometryToEigen( state_estimate_quad_.pose.pose.orientation )
        );
    }
    ...
    void DroneRacing::updateWaypointGoals() 
    {
        ...
        //    next_des_state_mb.velocity.norm() / global_trajectory_->getMaxVelocity() * max_velocity_, 
        //    max_velocity_
        //);

        visualizer_->displayDebug (
            goal_positions_.at(curr_goal_index_).x(),
        ...
        Eigen::Vector3d dist_to_next_gate = desired_state_world_.position - Eigen::Vector3d (
            goal_positions_.at(curr_goal_index_).x(),
            goal_positions_.at(curr_goal_index_).y(),
            goal_positions_.at(curr_goal_index_).z()
        );
    ...
    void DroneRacing::loadParameters() 
    {
        ...
        quadrotor_common::getParam<double>      ("camera_fov_yaw",                  camera_semi_fov_rad_yaw_,         45.0,   pnh_);
        quadrotor_common::getParam<double>      ("camera_fov_pitch",                camera_semi_fov_rad_pitch_,       45.0,   pnh_);

        camera_semi_fov_rad_yaw_    *=    M_PI / 180.0;
        camera_semi_fov_rad_pitch_  *=    M_PI / 180.0;
