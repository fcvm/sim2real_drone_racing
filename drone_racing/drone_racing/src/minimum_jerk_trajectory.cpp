...
namespace drone_racing 
{
    ...
    bool MinimumJerkTrajectory::computeTrajectory (
    ...
    ) 
    {
        ...
        std::shared_ptr<RapidTrajectoryGenerator> traj = std::make_shared<RapidTrajectoryGenerator> (
            pos0, 
            vel0, 
            acc0, 
            gravity
        ...
        double execution_time = std::max (
            (posf - pos0).GetNorm2() / desired_speed,
            (posf - pos0).GetNorm2() / (start_state.velocity.norm() + 0.5)
        );

        execution_time = std::max (
            (posf - pos0).GetNorm2() / max_velocity_, 
            execution_time
        );
        ...
        if ( res == RapidTrajectoryGenerator::InputFeasible ) 
        {
            ...
            if ( 
                positionFeasible_ceiling == RapidTrajectoryGenerator::StateFeasible &&
    ...
    void MinimumJerkTrajectory::setBestTrajectory (
        const int traj_idx, 
        const bool is_mb
    ) 
    {
