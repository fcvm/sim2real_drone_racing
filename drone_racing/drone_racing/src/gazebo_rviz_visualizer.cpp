...
// public: Con- and destructor
    ...
    GazeboRvizVisualizer::GazeboRvizVisualizer (
        ...
        pnh_(nh_private) 
    {
        ...
        gazebo_model_states_sub_ = nh_.subscribe (
            "/gazebo/model_states", 
            1, 
            &GazeboRvizVisualizer::gazeboModelStatesSub, 
            this
        );

        gate_publisher_ = nh_.advertise<visualization_msgs::MarkerArray> (
            "rviz_gates", 
            1
        );

        ROS_INFO (
            "[%s] Gazebo to RVIZ visualizer started!", 
...
// public: Methods
    ...
    void GazeboRvizVisualizer::visualizeGates() 
    {
        ...
        for (
            std::size_t i = 0, max = model_states_temp.name.size(); 
            i < max; 
            i++
        ) 
        {
            ...
            {
                next_position = quadrotor_common::geometryToEigen( model_states_temp.pose[i].position );
                gates_positions_.push_back( next_position );

                next_orientation = quadrotor_common::geometryToEigen( model_states_temp.pose[i].orientation );
        ...
        ) 
        {
            ...
            gate_marker.pose.orientation = quadrotor_common::eigenToGeometry( gates_orientations_[i] );
