...
// public: Methods
    ...
    bool TrajectoryManager::computeTrajectory (
    ...
    {
        double desired_velocity = std::max (
            max_velocity_ * network_selection.z(), 
            min_velocity_
        );

        double planning_length_rescaled = std::min (
            planning_length_max_,
            std::max (
                planning_length_min_, 
                planning_length_ * desired_velocity
            )
        );
        ...
        bool success = trajectory_implementation_->computeTrajectory (
            start_state,
            end_state,
        ...
        {
            ROS_ERROR (
                "[%s] Failed to compute NN trajectory.", 
    ...
    void TrajectoryManager::setBestTraj (
        const int best_traj_idx, 
        const bool is_mb
    ) 
    {
        trajectory_implementation_->setBestTrajectory (
            best_traj_idx, 
            is_mb
        );
        trajectory_implementation_->clearTrajectories( is_mb );
    ...
    std::vector<quadrotor_common::TrajectoryPoint> TrajectoryManager::sampleTrajectory(
    ...
    quadrotor_common::TrajectoryPoint TrajectoryManager::getDesiredStateFromTrajectory() 
    {
